...
public class MockDataService {
    ...
    protected List<Product> seedProducts(Integer companyId, String prefix,
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            // Numeric SKU/Barcode: Category * 1000 + index within category
            String numericCode = String.valueOf((catIdx + 1) * 1000 + (i % 4));
    ...
    protected List<Staff> seedStaff(Integer companyId, String prefix, int operatingYears) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            // Simplified role assignment for mock data
            if ("MANAGER".equals(s.getWarehouseRole())) {
                roleRepo.findByRoleName("MANAGER").ifPresent(staffRoles::add);
            } else if ("STOREKEEPER".equals(s.getWarehouseRole())) {
                roleRepo.findByRoleName("STOREKEEPER").ifPresent(staffRoles::add);
            } else {
