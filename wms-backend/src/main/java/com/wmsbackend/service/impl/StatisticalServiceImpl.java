...
public class StatisticalServiceImpl implements StatisticalService {
    ...
    public InventoryStatsDTO getInventoryStats(LocalDate startDate, LocalDate endDate) {
        ...
        // Lấy tổng nhập/xuất/điều chỉnh theo sản phẩm trong kỳ
        Map<Integer, Double> inboundMap = toProductMap(
                transactionRepository.sumInboundByProductInPeriod(startDT, endDT));
        
        // Xuất kho (QuantityChange là số âm)
        Map<Integer, Double> outboundMap = toProductMap(
                transactionRepository.sumOutboundByProductInPeriod(startDT, endDT));
        
        // Lấy các điều chỉnh (ADJUSTMENT)
        Map<Integer, Double> adjustmentMap = toProductMap(
                transactionRepository.sumAdjustmentsByProductInPeriod(startDT, endDT));
        ...
        for (Product p : products) {
            ...
            // Tồn đầu kỳ = Tồn cuối kỳ - Nhập + Xuất - Điều chỉnh
        ...
        // 1. Hao hụt từ QC (Hàng hỏng khi nhập)
        ...
        // 2. Hao hụt từ Kiểm kê (Điều chỉnh âm)
        transactionRepository.findAll().stream() // Giữ tạm findAll vì quy mô nhỏ, sẽ optimize sau nếu cần
    ...
    private List<InventoryStatsDTO.AbcItem> calculateABC(List<InventoryStatsDTO.ProductStockSummary> summaries) {
        ...
        if (totalValue <= 0) {
            return List.of(new InventoryStatsDTO.AbcItem("A", 0, 0, 0), new InventoryStatsDTO.AbcItem("B", 0, 0, 0), new InventoryStatsDTO.AbcItem("C", 0, 0, 0));
        }
        ...
        for (InventoryStatsDTO.ProductStockSummary s : sorted) {
            ...
            if (pct <= 80) {
                s.setAbcClass("A"); aCount++; aValue += s.getStockValue();
            } else if (pct <= 95) {
                s.setAbcClass("B"); bCount++; bValue += s.getStockValue();
            } else {
                s.setAbcClass("C"); cCount++; cValue += s.getStockValue();
            }
        }
