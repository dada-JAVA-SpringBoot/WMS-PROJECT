...
export default function CycleCounting() {
    ...
    // New states for Assignment logic
    ...
    return (
        <div className="p-4 md:p-6 bg-[#f8f9fa] min-h-full flex flex-col text-left font-sans text-gray-800 no-scrollbar">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 md:gap-8 flex-1">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar p-3 md:p-4 space-y-3">
                        {plans.sort((a,b) => b.id - a.id).map(plan => (
                            <div 
                                ...
                                className={`p-3 md:p-4 rounded-xl md:rounded-2xl border-2 transition-all cursor-pointer ${activePlan?.id === plan.id ? 'border-[#1192a8] bg-cyan-50' : 'border-gray-50 hover:border-gray-200'}`}
                            >
                ...
                <div className="xl:col-span-2 bg-white rounded-2xl md:rounded-3xl border border-gray-100 shadow-sm flex flex-col h-[500px] md:h-[700px] overflow-hidden">
                    {!activePlan ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-gray-300 gap-4 p-8 text-center">
                            ...
                            <div className="flex-1 overflow-x-auto no-scrollbar lg:scrollbar-thin">
                                <table className="w-full text-left min-w-[700px]">
                                    ...
                                    <tbody className="text-xs md:text-sm">
                                        {details.map((d) => (
                                            <tr key={d.id} className={`border-b border-gray-50 transition-colors ${d.variance !== 0 ? 'bg-orange-50/30' : 'hover:bg-gray-50/50'}`}>
                                                <td className="p-4">
                                                    <span className="font-black text-[#1192a8] uppercase">{d.binCode}</span>
                                                    <div className="text-[8px] md:text-[9px] text-gray-400 font-bold uppercase">{d.zone}</div>
                                                </td>
                                                <td className="p-4">
                                                    <div className="font-bold text-gray-800 uppercase truncate max-w-[150px]" title={d.productName}>{d.productName}</div>
                                                    <div className="text-[9px] md:text-[10px] font-mono text-gray-400">{d.productSku}</div>
                                                </td>
                                                <td className="p-4 font-mono font-bold text-gray-500">{d.batchCode}</td>
                                                <td className="p-4 text-center font-bold text-gray-400">{d.systemQty.toLocaleString()}</td>
                                                <td className="p-4">
                                                    <input 
                                                        type="number" 
                                                        value={d.countedQty} 
                                                        ...
                                                        className={`w-full text-center py-2 px-1 rounded-xl border-2 font-black transition-all outline-none text-sm ${d.variance === 0 ? 'border-gray-100 focus:border-[#1192a8]' : 'border-orange-200 text-orange-600 bg-white shadow-sm focus:border-orange-400'}`}
                                                    />
                                                </td>
