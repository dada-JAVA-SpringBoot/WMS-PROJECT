...
const CONTRACT_MAP = {
    ...
    INTERN:     { label: 'Thực tập sinh',   color: 'bg-pink-100 text-pink-700' },
};
...
export default function Staff({ onCreateInbound, onCreateOutbound }) {
    ...
    // ── Client-side filter (status + role + searchBy) ──────────────────────
            ...
            <div className="flex items-center justify-between bg-white p-5 rounded-3xl shadow-sm border border-gray-100 mb-6">
                ...
                <div className="flex items-center gap-3 flex-wrap justify-end">
                    ...
                    <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
                        className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1192a8]/20 bg-white text-gray-600 cursor-pointer">
                    ...
                    <button onClick={() => { setSearch(''); setFilterStatus('all'); setFilterRole('all'); fetchData(''); }}
                        className="bg-[#1192a8] text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-teal-700 hover:shadow-lg hover:shadow-teal-500/30 flex items-center gap-2 transition-all active:scale-95">
                            ...
                            {filtered.length === 0 ? (
                                ...
                                <tr key={row.id}
                                    ...
                                    className={`transition-colors cursor-pointer group
                                        ${row.workStatus === 'RESIGNED' ? 'opacity-50' : ''}
                                        ${selected?.id === row.id
