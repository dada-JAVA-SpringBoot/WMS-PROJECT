...
export default function WavePicking() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // Cả 2 API có thể trả về Page object { content: [...] } hoặc array thẳng
            setWaves(Array.isArray(wavesRes.data) ? wavesRes.data : (wavesRes.data?.content ?? []));
            const allOrders = Array.isArray(ordersRes.data) ? ordersRes.data : (ordersRes.data?.content ?? []);
            setPendingOrders(allOrders.filter(o => o.status === 'ALLOCATED'));
        } catch (error) {
    ...
    return (
        <div className="p-4 md:p-6 bg-[#f8f9fa] dark:bg-gray-900 min-h-full flex flex-col text-left font-sans text-gray-800 dark:text-gray-100 no-scrollbar transition-colors duration-300">
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-6 md:mb-8 transition-all">
                <div>
                    <h1 className="text-xl md:text-2xl font-black text-gray-800 dark:text-gray-100 uppercase tracking-tight">{t('pages.WavePicking.title')}</h1>
                    <p className="text-gray-500 dark:text-gray-400 text-xs md:text-sm font-medium">{t('pages.WavePicking.subtitle')}</p>
                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 md:gap-8 flex-1">
                ...
                <div className="xl:col-span-1 bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col h-[500px] md:h-[700px] transition-colors duration-300">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar p-3 md:p-4 space-y-3">
                        ...
                        {pendingOrders.length === 0 && <div className="py-20 text-center text-gray-300 dark:text-gray-600 italic text-sm">{t('pages.WavePicking.noPendingOrders')}</div>}
                    </div>
                </div>
