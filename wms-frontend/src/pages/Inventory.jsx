...
export default function Inventory() {
    ...
    const filteredProducts = useMemo(() => {
        ...
        return products.filter(item => {
            switch (searchType) {
                case 'Theo tên SP':   return item.name?.toLowerCase().includes(keyword);
                case 'Theo SKU':      return item.sku?.toLowerCase().includes(keyword);
                case 'Theo Mã vạch':  return item.barcode?.toLowerCase().includes(keyword);
                default: return (
        ...
        <div className="p-6 h-full flex flex-col bg-gray-50">
            <div className="flex items-center justify-between bg-white p-4 mb-4 rounded-xl shadow-sm border border-gray-100">
                <div className="flex gap-6">
                    <div onClick={() => setIsAddModalOpen(true)}><ActionButton iconSrc={addIcon} label="THÊM MỚI" /></div>
                    ...
                    <table className="w-full text-center text-sm">
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {filteredProducts.length > 0 ? filteredProducts.map(item => (
                                <tr key={item.id} onClick={() => setSelectedProduct(item)}
                                    ...
                                    <td className="p-4"><span className="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs border border-gray-200">{item.baseUnit}</span></td>
                                    ...
                                    <td className="p-4 font-bold text-right text-green-600">{item.totalStock?.toLocaleString()}</td>
                                </tr>
                            )) : (
                                <tr><td colSpan="6" className="p-8 text-gray-500 text-center">Không tìm thấy sản phẩm nào phù hợp với "{searchKeyword}".</td></tr>
                            )}
                        </tbody>
                    </table>
                )}
            </div>
            <ProductModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSuccess={fetchProducts} />
