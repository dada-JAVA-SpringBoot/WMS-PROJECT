            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-6 md:mb-8 transition-all">
                <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-xs md:text-sm font-medium">
                        Theo dõi thời gian ra vào, trễ giờ và làm thêm
                    </p>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 p-4 md:px-6 md:py-3 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex justify-between sm:justify-end gap-4 md:gap-8 shrink-0 transition-colors duration-300">
                    <div className="text-center px-2">
                        <p className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-0.5">Ngày công</p>
                    ...
                    <div className="text-center px-2 border-l border-gray-50 dark:border-gray-700">
                        <p className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-0.5">Trễ (lần)</p>
                        <p className="text-lg md:text-xl font-black text-red-500">{history.filter(h => h.lateMinutes > 0).length}</p>
                    </div>
                    <div className="text-center px-2 border-l border-gray-50 dark:border-gray-700">
                        <p className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-0.5">Tăng ca (phút)</p>
                    ...
                    <table className="w-full text-left border-collapse min-w-[800px] md:min-w-[1000px]">
                        ...
                        {loading ? (
                            ...
                            <tr key={idx} className="hover:bg-blue-50/30 dark:hover:bg-gray-700/30 transition-colors group cursor-default">
                                <td className="px-5 md:px-6 py-4 text-sm font-black text-gray-700 dark:text-gray-200">
                                    {formatDate(row.workDate)}
                                </td>
                                <td className="px-5 md:px-6 py-4 text-sm font-mono font-bold text-green-600 dark:text-green-400">
                                    {formatTime(row.checkInTime)}
                                </td>
                                <td className="px-5 md:px-6 py-4 text-sm font-mono font-bold text-orange-600 dark:text-orange-400">
                                ...
                                <td className="px-5 md:px-6 py-4 text-sm text-orange-600 dark:text-orange-400 font-black">
                                    {row.overtimeMinutes > 0
                                        ? `+${row.overtimeMinutes}p`
                                        : <span className="text-gray-300 dark:text-gray-600 font-bold">—</span>
                                    }
                                </td>
                                <td className="px-5 md:px-6 py-4 min-w-[200px]">
                                    {row.lateReason && (
                                        <p className="text-xs text-gray-600 dark:text-gray-300 font-bold bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-lg inline-block mb-1">
                                            {row.lateReason}
                                        </p>
                                    )}
