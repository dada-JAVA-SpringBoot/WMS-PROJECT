        ...
        try {
            ...
            if (canSeeInbound) {
                try {
                    ...
                    // API trả về Page object { content: [...], totalElements: ... } hoặc array thẳng
                    const inData = inRes.data;
                    setInboundOrders(Array.isArray(inData) ? inData : (inData?.content ?? []));
                } catch (e) {
            ...
            if (canSeeOutbound) {
                try {
                    ...
                    setOutboundOrders(Array.isArray(outData) ? outData : (outData?.content ?? []));
                } catch (e) {
                    console.warn("Lỗi tải danh sách phiếu xuất:", e.message);
                }
            }

            // 4. Assigned Cycle Counts — chỉ lấy plans chưa hoàn thành được giao cho user hiện tại
            const canSeeCounting = roles.some(r => ['ADMIN', 'MANAGER', 'STOREKEEPER', 'WAREHOUSE_KEEPER', 'CHECKER'].includes(r));
            if (canSeeCounting && user?.id) {
                try {
                    const ccRes = await axiosClient.get(`/api/cycle-counts/my-active?staffId=${user.id}`);
                    setCycleCounts(Array.isArray(ccRes.data) ? ccRes.data : (ccRes.data?.content ?? []));
                } catch (e) {
        ...
        <div className="min-h-full bg-[#f8fafc] dark:bg-gray-900 p-6 lg:p-8 relative transition-colors duration-300">
            ...
            {dashboardType === 'OPERATIONS' && (
                <OperationsDashboard
                    roles={roles}
                    stats={stats}
                    inboundOrders={(Array.isArray(inboundOrders) ? inboundOrders : []).filter(o => ['DRAFT', 'PENDING',
                        'ORDERED'].includes(o.status))}
                    outboundOrders={(Array.isArray(outboundOrders) ? outboundOrders : []).filter(o => ['DRAFT', 'ALLOCATED',
                        'PICKING'].includes(o.status))}
