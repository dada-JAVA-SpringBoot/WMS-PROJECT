...
const outboundStatusOptions = [
    { value: 'DRAFT', label: 'Bản nháp', color: 'bg-gray-100 text-gray-500 border-gray-200 dark:bg-gray-700 dark:text-gray-400 dark:border-gray-600' },
    { value: 'ALLOCATED', label: 'Đã phân bổ', color: 'bg-blue-50 text-blue-700 border-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800' },
    { value: 'PENDING', label: 'Đang duyệt', color: 'bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800' },
    { value: 'COMPLETED', label: 'Đã xuất kho', color: 'bg-green-50 text-green-700 border-green-100 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800' },
...
export default function ExportReceipts({ workflow, clearWorkflow }) {
    ...
    const handleUpdateStatus = async (id, nextStatus) => {
        ...
        if (nextStatus === 'COMPLETED') {
            ...
            if (!canApprove) { showMsg("Từ chối", "Bạn không có quyền chuyển trạng thái sang 'Đã xuất kho'.", "info"); return; }
            try {
                ...
                return;
            } catch (error) { showMsg("Lỗi", "Không thể tải chi tiết phiếu để kiểm định", "info"); return; }
        }
    ...
    const handleConfirmQCResult = async (inspectionData) => {
        ...
        } catch (error) { showMsg("Lỗi", "Lỗi phê duyệt: " + (error.response?.data || error.message), "info"); }
    };
    ...
    const handleConfirmQC = async (id) => {
        try {
            ...
            setIsViewDetailOpen(false); fetchData();
            showMsg("Thành công", "Đã phê duyệt QC và xuất kho thành công!", "info");
        } catch (error) { showMsg("Lỗi", "Lỗi phê duyệt: " + (error.response?.data || error.message), "info"); }
    };
    ...
    const toolbarActions = [
        ...
        { label: 'XUẤT EXCEL', iconSrc: excelIcon, onClick: () => { const bestMode = detectBestExportMode(selectedIds.length, filteredData.length); openExportModal(bestMode); }},
    ...
    return (
        <div className="p-6 bg-[#f8f9fa] dark:bg-gray-900 min-h-full flex flex-col text-left font-sans transition-colors duration-300" onContextMenu={e => e.preventDefault()}>
            ...
            <ExportExcelModal isOpen={isExportModalOpen} onClose={closeExportModal} onConfirm={handleExportExcel} fileName={exportFileName} setFileName={setExportFileName} />
            <VoucherContextMenu isOpen={!!contextMenu} x={contextMenu?.x} y={contextMenu?.y} title="Thao tác nhanh" subtitle={contextMenu?.item?.issueCode} actions={contextActions} onClose={() => setContextMenu(null)} />
            ...
            <div className="sticky top-0 z-20 flex items-center justify-between bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm p-4 md:p-5 rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 mb-4 md:mb-6 transition-colors duration-300">
                ...
                <h1 className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest hidden lg:block">Hệ thống phiếu xuất kho</h1>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 md:p-5 rounded-2xl md:rounded-3xl border border-gray-100 dark:border-gray-700 mb-4 md:mb-6 flex flex-col gap-4 shadow-sm transition-colors duration-300">
                <input
                    ...
                    className="w-full border-2 border-gray-100 dark:border-gray-600 rounded-xl px-4 md:px-5 py-2.5 md:py-3 text-sm outline-none focus:border-[#1192a8] bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 transition-all"
                />
                <div className="flex flex-wrap gap-x-6 gap-y-3 border-t border-gray-100 dark:border-gray-700 pt-4">
                    <div className="flex items-center gap-2">
                        <span className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Khách hàng:</span>
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Người lập:</span>
            ...
            <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm flex flex-col transition-colors duration-300">
                <div className="overflow-x-auto flex-1 no-scrollbar lg:scrollbar-thin">
                    <table className="w-full text-left min-w-[800px] md:min-w-[1000px]">
                        ...
                        {isLoading ? (
                            <tr><td colSpan="6" className="py-20 text-center animate-pulse text-gray-400 dark:text-gray-500">ĐANG TẢI...</td></tr>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[95vh] md:max-h-[90vh] shadow-2xl transition-colors duration-300">
                        ...
                        <div className="p-4 md:p-8 overflow-y-auto flex-1 space-y-6 md:space-y-8 bg-gray-50/30 dark:bg-gray-900/30">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-10 border-b border-gray-100 dark:border-gray-700 pb-6 md:pb-8">
                                <div><p className="text-[9px] md:text-[10px] text-gray-400 dark:text-gray-500 font-black uppercase">Khách hàng</p><p className="text-base md:text-lg font-black text-gray-800 dark:text-gray-100">{getCustName(viewingVoucher.customerId)}</p></div>
                                <div><p className="text-[9px] md:text-[10px] text-gray-400 dark:text-gray-500 font-black uppercase">Thời gian tạo</p><p className="text-base md:text-lg font-black text-gray-800 dark:text-gray-100">{viewingVoucher.createdAt ? new Date(viewingVoucher.createdAt).toLocaleString('vi-VN') : '---'}</p></div>
                            </div>
                            <div className="overflow-x-auto no-scrollbar">
                                <table className="w-full text-xs md:text-sm text-left min-w-[500px]">
                                    ...
                                    {viewingVoucher.items?.map((row, i) => {
                                        ...
                                        return (
                                            <tr key={i}>
                                                <td className="p-3 font-bold text-left text-gray-800 dark:text-gray-200"><p>{p?.name || `SP #${row.productId}`}</p></td>
                        ...
                        <div className="p-4 md:p-5 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 flex justify-between items-center shrink-0 transition-colors duration-300">
                            <div className="flex gap-2">
                                {['PENDING', 'ALLOCATED', 'DRAFT'].includes(viewingVoucher.status) && (
                                    <button onClick={() => handleConfirmQC(viewingVoucher.id)} className="px-4 py-2 bg-rose-600 text-white rounded-xl font-bold uppercase text-[10px] shadow-lg shadow-rose-600/20 hover:scale-105 transition-all">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl w-full max-w-[98%] md:max-w-[95%] overflow-hidden flex flex-col max-h-[98vh] md:max-h-[95vh] shadow-2xl transition-colors duration-300">
                        ...
                        <div className="p-4 md:p-8 overflow-y-auto flex-1 space-y-6 md:space-y-8 bg-gray-50/50 dark:bg-gray-900/30">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm text-left">
                                <div>
                                    <label className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase block mb-1">Khách hàng *</label>
                                ...
                                <div>
                                    <label className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase block mb-1">Người xuất</label>
                                    <select value={formData.staffId} onChange={e => setFormData({...formData, staffId: e.target.value})} className="wms-select !py-2 !w-full !text-xs md:!text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
                                        <option value="">-- Chọn --</option>{filteredStaffs.map(s => <option key={s.id} value={s.id}>{s.fullName}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase block mb-1">Mã phiếu</label>
                                ...
                                <div>
                                    <label className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase block mb-1">Ngày lập</label>
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    <h3 className="text-[10px] md:text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest text-left">Hàng hóa</h3>
                                    <button onClick={() => setDetails([...details, createEmptyDetail()])} className="text-[9px] md:text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase hover:underline">+ THÊM DÒNG</button>
                                </div>
                                <div className="bg-white dark:bg-gray-800 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700 overflow-x-auto no-scrollbar text-left">
                                    <table className="w-full text-[10px] md:text-xs text-left min-w-[1000px]">
                                        ...
                                        <tr className="bg-gray-50 dark:bg-gray-700/50 text-gray-400 dark:text-gray-500 font-bold uppercase">
                                            <th className="p-3">Sản phẩm</th>
                                            <th className="p-3">Lô & Vị trí gợi ý (FEFO)</th>
                                            <th className="p-3 text-center w-20 md:w-24">SL</th>
                                            <th className="p-3 text-right w-28 md:w-32">Giá</th>
                        ...
                        <div className="p-4 md:p-6 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 flex flex-col sm:flex-row justify-between items-stretch sm:items-center shrink-0 gap-4 transition-colors duration-300">
                            <div className="text-left">
                                <p className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase">Tổng giá trị</p>
                                <p className="text-xl md:text-2xl font-black text-[#1192a8]">{details.reduce((s, r) => s + r.total, 0).toLocaleString()}đ</p>
                            </div>
                            <div className="flex gap-4">
                                <button onClick={() => setIsCreateOpen(false)} className="flex-1 sm:flex-none text-gray-400 dark:text-gray-500 font-bold uppercase text-[10px] md:text-xs hover:text-gray-600 dark:hover:text-gray-300">Hủy</button>
                                <button onClick={handleSave} className="flex-1 sm:flex-none px-6 md:px-10 py-2.5 md:py-3 bg-[#1192a8] text-white rounded-xl md:rounded-2xl font-black uppercase text-[10px] md:text-xs shadow-lg shadow-[#1192a8]/20 transition-all hover:scale-105 active:scale-95">Xác nhận phiếu</button>
                            </div>
                        </div>
                    </div>
                </div>
