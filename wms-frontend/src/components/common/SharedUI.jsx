                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <div className="flex gap-2">
                        <select className="flex-1 sm:flex-none border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1192a8]/20 focus:border-[#1192a8] bg-white text-gray-600 cursor-pointer">
                            <option>{isEnglish ? 'All' : 'Tất cả'}</option>
                            <option>{isEnglish ? 'By name' : 'Theo tên'}</option>
                            <option>{isEnglish ? 'By code' : 'Theo mã'}</option>
                        </select>
                        <div className="relative flex-1 sm:flex-none">
                            <input
                                ...
                                placeholder={isEnglish ? 'Enter search text...' : 'Nhập nội dung tìm kiếm...'}
                            />
                        </div>
                    </div>
                    <button
                    ...
                    >
                        <span className="text-lg leading-none">↻</span> {isEnglish ? 'Refresh' : 'Làm mới'}
                    </button>
                </div>
            )}
        </div>
