...
export default function SmartAssistant({ stats, inboundOrders, outboundOrders, cycleCounts }) {
    ...
    const analysis = useMemo(() => {
        ...
        if (roles.includes('STOREKEEPER') || roles.includes('WAREHOUSE_KEEPER') || roles.includes('INVENTORY_CHECKER') || roles.includes('CHECKER') || roles.includes('HANDLER')) {
            const safeCycleCounts = Array.isArray(cycleCounts) ? cycleCounts : [];
            if (safeCycleCounts.length > 0) {
                tasks.push(isEnglish ? `Complete ${safeCycleCounts.length} assigned cycle counts.` : `Hoàn thành ${safeCycleCounts.length} đợt kiểm kê được giao.`);
        ...
        if (roles.includes('INBOUND_STAFF') || roles.includes('OUTBOUND_STAFF') || roles.includes('QUALITY_CONTROL')) {
            const safeInboundOrders = Array.isArray(inboundOrders) ? inboundOrders : [];
            const safeOutboundOrders = Array.isArray(outboundOrders) ? outboundOrders : [];

            const pendingIn = safeInboundOrders.filter(o => o.status === 'PENDING' || o.status === 'DRAFT' || o.status ===
                'ORDERED').length;
            const pendingOut = safeOutboundOrders.filter(o => o.status === 'ALLOCATED' || o.status === 'PICKING' || o.status
                === 'DRAFT').length;

            if (roles.includes('QUALITY_CONTROL')) {
                const needsQC = safeInboundOrders.filter(o => o.status === 'PENDING' || o.status === 'ORDERED').length;
