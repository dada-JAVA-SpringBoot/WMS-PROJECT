        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm h-[450px] flex flex-col">
                    <h3 className="font-bold text-gray-800 mb-6">Dòng chảy hàng hóa (7 ngày)</h3>
                    <div className="flex-1 min-h-0">
                        <LineAreaChart 
                            labels={chartData.map(d => d.label)}
                            series={[
                                { label: 'Nhập kho', data: chartData.map(d => d.inbound), color: '#1192a8', fill: '#1192a8' },
                                { label: 'Xuất kho', data: chartData.map(d => d.outbound), color: '#f43f5e', fill: '#f43f5e' }
                            ]}
                        />
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm h-[450px] flex flex-col">
                    <h3 className="font-bold text-gray-800 mb-6">Cơ cấu tồn kho theo danh mục</h3>
                    <div className="flex-1 min-h-0">
                        <GroupedBarChart 
                            labels={stats?.stockByCategory?.map(c => c.category) || []}
                            series={[
                                { label: 'Số lượng', data: stats?.stockByCategory?.map(c => c.totalStock) || [], color: '#1192a8' }
                            ]}
                        />
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="md:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="p-4 border-b border-gray-50 bg-gray-50/50">
                        <h3 className="font-bold text-gray-800">Top 5 sản phẩm tồn kho nhiều nhất</h3>
                    </div>
                ...
                <div className="bg-gradient-to-br from-[#1192a8] to-[#0e7c8a] p-8 rounded-2xl text-white shadow-xl shadow-teal-100 flex flex-col justify-center">
                    <div className="mb-6">
                        <h3 className="text-xl font-bold mb-2">Tóm lược vận hành</h3>
                        <p className="text-teal-50 text-sm opacity-80">Dữ liệu được cập nhật thời gian thực từ các trạm làm việc.</p>
                    </div>
                    <div className="space-y-4">
                        <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
                            <div className="text-xs uppercase font-bold opacity-60 mb-1">Tỉ lệ lấp đầy</div>
                            <div className="text-3xl font-black">{stats?.warehouseOccupancyRate || 0}%</div>
                        </div>
                        <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
                            <div className="text-xs uppercase font-bold opacity-60 mb-1">Mã hàng thấp hơn định mức</div>
